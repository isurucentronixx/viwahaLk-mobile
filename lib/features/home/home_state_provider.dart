...
class SearchResultNotifier  extends StateNotifier<List<SearchResultItem>> {
  ...
  SearchResultNotifier({required this.ref}) : super([])  {
    fetchSearchResult(ref: ref);
  }

  Future fetchSearchResult({required Ref ref}) async {
    String location = ref.watch(selectedSubLocationProvider).sub_location_en ?? ref.watch(selectedMainLocationProvider);
    String category = ref.watch(selectedSubCategoryProvider).sub_category ?? ref.watch(selectedMainCategoryProvider);
    String keyword = "";
